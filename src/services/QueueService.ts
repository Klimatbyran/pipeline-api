...
import startQueues, { JOB_STATUS, QUEUE_NAMES } from "../lib/bullmq";
...
export class QueueService {
  ...
  public async getQueue(name: string) {
    if (this.queues === undefined) this.queues = await this.getQueues();
  ...
  public async getQueueStats(queueName?: string): Promise<QueueStatus[]> {
    const queues = queueName
      ...
      : Object.values(await this.getQueues());
  ...
  public async rerunJob(
    ...
    console.info("[QueueService] rerunJob: Starting", {
      queueName,
      jobId,
      hasDataOverrides:
        !!dataOverrides && Object.keys(dataOverrides).length > 0,
    });
    ...
    if (!job) {
      console.error("[QueueService] rerunJob: Job not found", {
        queueName,
        jobId,
      });
    ...
    console.info("[QueueService] rerunJob: Job state", {
      queueName,
      jobId,
      currentState,
      jobName: job.name,
    ...
    if (dataOverrides && Object.keys(dataOverrides).length > 0) {
      console.info("[QueueService] rerunJob: Updating job data", {
        queueName,
        jobId,
        dataOverrides,
      });
      ...
      console.info("[QueueService] rerunJob: Job data updated", {
        queueName,
        jobId,
      });
    }
    ...
    if (currentState === "delayed") {
      console.info("[QueueService] rerunJob: Promoting delayed job", {
        queueName,
        jobId,
      });
      await job.promote();
      const newState = await job.getState();
      console.info("[QueueService] rerunJob: Job promoted", {
        queueName,
        jobId,
      ...
      console.info("[QueueService] rerunJob: Retrying failed job", {
        queueName,
        jobId,
      });
      await job.retry();
      const newState = await job.getState();
      console.info("[QueueService] rerunJob: Job retried", {
        queueName,
        jobId,
        previousState: currentState,
        newState,
      ...
      console.info(
        "[QueueService] rerunJob: Creating new job from completed job",
        { queueName, jobId, threadId: job.data?.threadId },
      );
      // For completed jobs, create new one with same data (preserving threadId)
      const newJob = await queue.add(job.name, job.data);
      console.info("[QueueService] rerunJob: New job created", {
        queueName,
        originalJobId: jobId,
        newJobId: newJob.id,
      });
      ...
      console.warn("[QueueService] rerunJob: Job already in runnable state", {
        queueName,
        jobId,
        currentState,
      });
      ...
      console.info("[QueueService] rerunJob: Moving job to waiting", {
        queueName,
        jobId,
        currentState,
      });
      ...
      const newState = await job.getState();
      console.info("[QueueService] rerunJob: Job moved to waiting", {
        queueName,
        jobId,
        previousState: currentState,
        newState,
    ...
    console.info("[QueueService] rerunJob: Completed", {
      queueName,
      jobId,
      finalState: finalJob.status,
    });
    return finalJob;
  }
  ...
  public async rerunExtractEmissionsFromFollowup(
    ...
    const followupJob = await this.getFollowupJob(
      followupQueueName,
      followupJobId,
    );
    ...
    console.info(
      "[QueueService] rerunExtractEmissionsFromFollowup: New job created",
      {
        newJobId: newJob.id,
        scopes,
      },
  ...
  private getThreadIdFromJob(job: DataJob): string {
    ...
    const threadId = followupData.threadId ?? job.threadId ?? job.processId;
    ...
    if (!threadId) {
      ...
      throw new Error(
        "Cannot locate process/thread for this job (no threadId).",
      );
    }
  ...
  private async getLatestExtractEmissionsJobForThread(
    threadId: string,
  ): Promise<DataJob> {
    ...
    if (!extractJobs.length) {
      console.error(
        "[QueueService] getLatestExtractEmissionsJobForThread: No EXTRACT_EMISSIONS job found",
        {
          threadId,
        },
      );
    ...
    return extractJobs.sort(
      (firstJob, secondJob) =>
        (secondJob.timestamp ?? 0) - (firstJob.timestamp ?? 0),
    )[0];
  }
  ...
  private getCompanyNameFromJobs(
    ...
    return extractData.companyName ?? followupData.companyName ?? threadId;
  }
  ...
  private async getLatestFiscalYearForThread(
    threadId: string,
    ...
    try {
      ...
      if (typeof returnValue === "string") {
        try {
          ...
          console.warn(
            "[QueueService] getLatestFiscalYearForThread: Failed to parse fiscalYear returnvalue",
            {
              threadId,
              error: parseErr,
            },
      ...
      console.warn(
        "[QueueService] getLatestFiscalYearForThread: Failed to fetch FOLLOW_UP_FISCAL_YEAR jobs",
        {
          threadId,
          error: err,
        },
  ...
  private async getWikidataFromGuessWikidataJob(
    threadId: string,
  ): Promise<any | undefined> {
    try {
      ...
      if (guessWikidataJobs.length === 0) {
        console.info(
          "[QueueService] getWikidataFromGuessWikidataJob: No completed GUESS_WIKIDATA jobs found",
          { threadId },
        );
        return undefined;
      }
      ...
      console.warn(
        "[QueueService] getWikidataFromGuessWikidataJob: Failed to fetch GUESS_WIKIDATA jobs",
        {
          threadId,
          error: err,
        },
  ...
  private async buildWikidataCache(
    ...
    try {
      ...
      console.info("[QueueService] buildWikidataCache: Built cache", {
        ...
        withWikidata: Array.from(wikidataByThreadId.values()).filter(Boolean)
          .length,
      });
      ...
      console.warn(
        "[QueueService] buildWikidataCache: Failed to build wikidata cache",
        {
          error,
        },
      );
  ...
  private async buildExtractEmissionsJobCache(
    ...
    const extractJobs = await this.getDataJobs([QUEUE_NAMES.EXTRACT_EMISSIONS]);
    ...
    for (const extractJob of extractJobs) {
      ...
      const extractThreadId: string | undefined =
        extractData.threadId ?? extractJob.threadId ?? extractJob.processId;

      if (!extractThreadId || !uniqueThreadIds.has(extractThreadId)) {
        continue;
      }

      const existingJobForThread =
        extractEmissionsByThreadId.get(extractThreadId);
  ...
  private extractFiscalYearFromReturnValue(returnValue: any): any | undefined {
    if (typeof returnValue === "string") {
      try {
        ...
        console.warn(
          "[QueueService] extractFiscalYearFromReturnValue: Failed to parse fiscalYear returnvalue",
          {
            error: parseErr,
          },
        );
  ...
  private async buildFiscalYearCache(
    ...
    try {
      ...
      for (const fiscalJob of fiscalJobs) {
        this.addFiscalYearForJobToCache(
          fiscalJob,
          targetThreadIds,
      ...
      console.warn(
        "[QueueService] buildFiscalYearCache: Failed to build fiscal year cache",
        {
          error,
        },
      );
  ...
  private addFiscalYearForJobToCache(
    ...
    const fiscalYear = this.extractFiscalYearFromReturnValue(
      fiscalJob.returnvalue,
    );
  ...
  private getThreadIdFromFiscalJob(fiscalJob: DataJob): string | undefined {
    ...
    return fiscalData.threadId ?? fiscalJob.threadId ?? fiscalJob.processId;
  }
  /**
  ...
  public async rerunJobsByWorkerName(
    ...
    const queueNames =
      options?.queueNames && options.queueNames.length > 0
        ? options.queueNames
    ...
    console.info(
      "[QueueService] rerunJobsByWorkerName: Starting (using rerun-and-save)",
      {
        workerName,
        queueNames,
        statuses,
    ...
    const { allMatchingJobInfos, perQueue, totalMatched } =
      await this.collectMatchingJobsByWorker(workerName, queueNames, statuses);

    const jobsToRerun =
      this.selectLatestThreadIdPerCompany(allMatchingJobInfos);
    const limitedJobsToRerun =
      limit === "all"
        ? jobsToRerun
        : this.limitToMostRecent(jobsToRerun, limit);

    const threadIdsToRerun = limitedJobsToRerun.map(
      (jobInfo) => jobInfo.threadId,
    );

    const extractEmissionsByThreadId =
      await this.buildExtractEmissionsJobCache(threadIdsToRerun);
    const fiscalYearByThreadId =
      await this.buildFiscalYearCache(threadIdsToRerun);
    ...
    console.info(
      "[QueueService] rerunJobsByWorkerName: Completed (using rerun-and-save)",
      {
        workerName,
        totalMatched,
  ...
  private async collectMatchingJobsByWorker(
    ...
    const allMatchingJobInfos: Array<{
      job: Job;
      queueName: string;
      companyName: string;
      threadId: string;
  ...
  private async rerunSelectedJobs(
    ...
    console.info(
      "[QueueService] rerunJobsByWorkerName: Deduplicated and limited",
      {
    ...
    for (const jobInfo of jobInfos) {
      try {
        ...
        console.error(
          "[QueueService] rerunJobsByWorkerName: Failed to rerun and save job",
          {
