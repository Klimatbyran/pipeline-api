...
import {
  ...
  readQueuePathParamsSchema,
  readQueueQueryStringSchema,
  readQueueStatsQueryStringSchema,
...
const DEFAULT_PARSE_PDF_CONCURRENCY = Number.parseInt(
  process.env.PARSE_PDF_ENQUEUE_CONCURRENCY ?? "4",
  10,
);

function getParsePdfConcurrency(): number {
  if (!Number.isFinite(DEFAULT_PARSE_PDF_CONCURRENCY)) return 4;
  return Math.max(1, Math.min(16, DEFAULT_PARSE_PDF_CONCURRENCY));
}
...
> {
  ...
  type UploadEnqueueResult =
    | {
        ok: true;
        job: BaseJob;
        upload: { filename: string } & PdfUploadResult;
      }
    | {
        ok: false;
        status: 413 | 500;
  ...
  const concurrency = getParsePdfConcurrency();
  const results = await mapWithConcurrency(
    files,
    concurrency,
    async ({ buffer, filename }): Promise<UploadEnqueueResult> => {
  ...
  app.post(
    ...
    async (
      ...
      if (resolvedName === QUEUE_NAMES.PARSE_PDF && cachePdf) {
        ...
        const results = await mapWithConcurrency(
          ...
          async (url) => {
            try {
              ...
              return { ok: true as const, entry, job: addedJob };
            } catch (err: any) {
              ...
              request.log.warn(
                { err, url },
                "parsePdf cachePdf failed for URL",
              );
        ...
        for (const result of results) {
          if (result.ok) {
            cached.push(result.entry);
            addedJobs.push(result.job);
          } else {
            cacheErrors.push({ url: result.url, error: result.error });
          }
        }
      } else {
        const jobs = await mapWithConcurrency(
          ...
          async (url) => {
            const perUrlThreadId = randomUUID();
            return queueService.addJob(resolvedName, url, autoApprove, {
              forceReindex,
              threadId: perUrlThreadId,
              replaceAllEmissions,
              runOnly,
